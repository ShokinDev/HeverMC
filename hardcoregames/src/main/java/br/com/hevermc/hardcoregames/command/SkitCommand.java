		...
		if (isPlayer(sender)) {
			...
			if (hasGroup(p, Groups.MODPLUS, true)) {
				if (args.length == 0) {
					...
					p.sendMessage("§aVocê deve utilizar §e/skit criar <kit>!");
					p.sendMessage("§aVocê deve utilizar §e/skit aplicar <kit> <blocos>!");
					return true;
				}
				if (args[0].equalsIgnoreCase("criar")) {
					...
					if (!this.itens.containsKey(nome) && !this.armor.containsKey(nome)) {
						p.playSound(p.getLocation(), Sound.CLICK, 15.0F, 1.0F);
						p.sendMessage("§cO kit §4" + nome + "§c não foi encontrado!");
						return true;
					}
					if (this.isInt(args[2])) {
						...
						for (final Entity e : p.getNearbyEntities((double) n, (double) n, (double) n)) {
							if (e instanceof Player) {
								...
								p.playSound(p.getLocation(), Sound.LEVEL_UP, 15.0F, 1.0F);
								p2.sendMessage("§aO kit §e" + nome + "§a foi definido para você!");
								p2.setAllowFlight(false);
							}
						}
						p.playSound(p.getLocation(), Sound.LEVEL_UP, 15.0F, 1.0F);
						p.sendMessage(
								"§aVocê aplicou o kit §e" + nome + " §aem uma distancia de§e " + n + "§a bloco(s)!");
						return true;
					}
				}
			}
