				...
				if (args.length == 0) {
					...
					if (args[0].equalsIgnoreCase("on")) {
						if (KitPvP.getManager().eventOcurring == true) {
							p.sendMessage("§cO evento já está ativo!");
						} else {
							p.sendMessage("§eVocê ativou o evento!");
						...
						if (KitPvP.getManager().eventOcurring == false) {
							p.sendMessage("§cVocê só pode executar esse comando com o evento ativado!");
						} else {
							KitPvP.getManager().buildInEvent = !KitPvP.getManager().buildInEvent;
							p.sendMessage("§aVocê " + (KitPvP.getManager().buildInEvent ? "§eativou" : "§cdesativou")
						...
						p.sendMessage("§cVocê finalizou o evento!");
					}
				}
			} else {
				if (pvp.isCombat()) {
